...
if (class_exists("Blackjack")) {
  ...
  class Blackjack extends EzBasePlugin {
    ...
    var $slug, $plgDir, $plgURL, $ezAdmin, $myPlugins;
    var $adminMsg = '';
    ...
    function displayGame($atts, $content = '') {
      ...
      $game = "<div id='blackjack'>
       <h2>{$this->title}</h2><div class='page'>
...
<div>";
    ...
    function printAdminPage() {
      ...
      <div class="wrap" style="width:1000px">
        <h2><?php echo "{$this->name}{$this->strPro}"; ?><a href="http://validator.w3.org/" target="_blank"><img src="http://www.w3.org/Icons/valid-xhtml10" alt="Valid HTML5" title="<?php echo "{$this->name}{$this->strPro}"; ?> Admin Page is certified Valid HTML5" height="31" onmouseover="Tip('<?php echo "{$this->name}{$this->strPro}"; ?> Admin Page is certified Valid HTML5, with no errors in the HTML code generated by the plugin.')" onmouseout="UnTip()" width="88" class="alignright"/></a>
        </h2>
        <br>
        <form method="post">
          ...
          <table style="width:95%">
            ...
            <tr><td colspan="3">
                ...
                <label for="showCredit" style="color:#e00;" onmouseover="Tip('If you would like to support this plugin development, you can show a tiny credit link below the game display.', WIDTH, 240, TITLE, '', FIX, [this, 5, 5])" onmouseout="UnTip()"><input type="checkbox" id="showCredit"  name="showCredit" <?php echo $showCredit; ?> /> &nbsp; Show a tiny credit link at the bottom of the game.</label>
                <br /><b>
                  <label for='kill_author' onmouseover="Tip('If you find the author links and ads on the plugin admin page distracting or annoying, you can suppress them by checking this box. Please remember to save your options after checking.', WIDTH, 240, TITLE, '', FIX, [this, 5, 5])" onmouseout="UnTip()">
                    <input type='checkbox' id='kill_author' name='kill_author' <?php echo $kill_author; ?> /> &nbsp; Kill author links on the admin page?
                  </label>
                </b>
              </td></tr>
          </table>
          <div class="submit">
            <input class="button button-primary" type="submit" name="saveChanges" value="Save Changes" title="Save the changes as specified above" onmouseover="Tip('Save the changes as specified above', WIDTH, 240, TITLE, 'Save Changes')" onmouseout="UnTip()"/>
          </div>
        </form>
        <hr />
